...
export function Vaccine({ children }) {
  ...
  async function getList(page = 1, limit = 10) {
    try {
      axios
        .get(
          VACCNINE.LIST.replace(/:number/gi, page).replace(/:limit/gi, limit),
  ...
  async function remove(id) {
    try {
      const json = await axios.delete(VACCNINE.REMOVE.replace(/:id/gi, id), {
        headers: { Authorization: `Bearer ${token}` },
      });
      await getList();
    } catch (error) {
  ...
  async function create(params) {
    try {
      const json = await axios.post(VACCNINE.CREATE, params, {
        headers: { Authorization: `Bearer ${token}` },
      });
      await getList();
    } catch (error) {
  ...
  return (
    <VaccineContext.Provider
      value={{ list, create, remove, getList, pagination }}
    >
      {children}
    </VaccineContext.Provider>
  );
