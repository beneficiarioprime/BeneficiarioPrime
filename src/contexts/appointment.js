...
export function Appointment({ children }) {
  ...
  async function getList(page = 1, limit = 10) {
    try {
      axios
        .get(
          APPOINTMENT.LIST.replace(/:number/gi, page).replace(
            /:limit/gi,
            limit
          ),
  ...
  async function remove(id) {
    try {
      const json = await axios.delete(APPOINTMENT.REMOVE.replace(/:id/gi, id), {
        headers: { Authorization: `Bearer ${token}` },
      });
      await getList();
    } catch (error) {
  ...
  async function create(params) {
    try {
      const json = await axios.post(APPOINTMENT.CREATE, params, {
        headers: { Authorization: `Bearer ${token}` },
      });
      await getList();
    } catch (error) {
  ...
  return (
    <AppointmentContext.Provider
      value={{ list, remove, create, getList, pagination }}
    >
      {children}
    </AppointmentContext.Provider>
  );
